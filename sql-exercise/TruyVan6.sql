...
SELECT GV.MaGV, GV.HoTen, DT.TenDT FROM dbo.GIAOVIEN AS GV
	JOIN (SELECT TenDT, GVCNDT FROM dbo.DETAI) AS DT
	ON DT.GVCNDT = GV.MaGV
	
SELECT * FROM
dbo.GIAOVIEN AS GV, (SELECT * FROM dbo.DETAI) AS DT
...
WHERE 1 <
(
	SELECT COUNT(*) FROM dbo.THAMGIADT AS TGDT
